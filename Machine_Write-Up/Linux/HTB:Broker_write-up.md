...
1. 公開漏洞可直接取得系統控制權，腳本也不需要修改，非常強大。
2. 難得練習了nginx的提權，非常有趣，雖說nginx提權甚至可以直接讀取root.txt，不需取得root shell，但我依然堅持要練習取得。

### 浪費時間的部分：

1. 找到的全自動腳本有點卡卡的，重試幾次才成功。

### 新知識點：

1. 公開漏洞的邏輯還是需要了解:攻擊者送封包→封包裡有className→ActiveMQ載入class→ActiveMQ建立物件→物件初始化→執行攻擊者控制的內容。
2. 手動攻擊流程是:攻擊者建立client(Python or nc or ...)→連線→透過client發送OpenWire封包Payload→系統自動執行。

### 與實戰對應：

1. 反序列化是很嚴重的漏洞，遇到需仔細查看。
2. JAVA語言在靶機裡相對較少，實戰卻很多。
