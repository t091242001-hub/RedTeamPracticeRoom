...
### 主要發現：
1. 漏洞 ：利用舊版本的公開漏洞(CVE-2023-41425)透過跨站腳本(XSS)達成遠端程式碼執行(RCE)（風險等級：Medium）
2. 漏洞 ：利用未設過濾機制的root權限控制面板透過命令注入達成遠端程式碼執行(RCE)（風險等級：Medium）
...
攻擊鏈清晰，並無特別難的部分，公開漏洞可直接使用腳本，非常方便。

### 浪費時間的部分：

Searchsploit提供的腳本有少許語法錯誤，需花時間修改，或是線上取得別的腳本。
在使用腳本前必須理解腳本的整體邏輯，否則無法手動打出。
關於WonderCMS的資訊幾乎為Google搜尋得來，靶機內沒有太明顯提示。

### 新知識點：

使用相對大量Google搜尋，對WonderCMS以及公開漏洞搜尋的方法增加了。
手動漏洞利用邏輯有點複雜，理解後其實不難。

### 與實戰對應：

系統並未給太多提示，需要自己Google搜尋、修改腳本或是尋找其他更好用的腳本。
