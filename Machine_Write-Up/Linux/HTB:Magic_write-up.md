...
### 主要發現：
1. 漏洞 ：利用登入頁面的SQLi繞過身分驗證進入系統（風險等級：High）
2. 漏洞 ：透過可被繞過的白名單限制利用檔案上傳漏洞獲得系統存取權（風險等級：High）
3. 漏洞 ：透過讀取權限配置不當的資料庫文件獲取使用者憑證（風險等級：Medium）
4. 漏洞 ：透過擁有SUID權限的sysinfo檔利用路徑劫持達成權限提升（風險等級：High）

### 攻擊鏈摘要：

攻擊者首先利用登入頁面的SQLi繞過身分驗證進入系統，隨後繞過上傳白名單部署PHP WebShell取得初步控制權。進入系統後，藉由讀取權限設置不當的資料庫文件取得憑證並完成橫向移動。最後，利用具備SUID權限且含有路徑劫持漏洞的系統工具/bin/sysinfo，成功將權限提升至 Root。


### 潛在影響：

1. 完全失去系統控制權，攻擊者可取得伺服器最高管理權限。
2. 資料庫內的機密資訊面臨全面外洩。
...
2. 對上傳檔案進行嚴格的後端檢查。
3. 嚴格限制敏感配置文件的讀取權限。
4. 對具備SUID的程式進行審計，避免在popen()等函數中使用相對路徑，應改用絕對路徑。
...
1. 登入頁面的SQLi只需要最簡單的PoC即可通過。
2. 上傳白名單只要簡單的手動嘗試即可繞過，整體來說獲得立足點並不難。
3. 在讀取資料庫文件階段，連線下載mysql工具之前檢查了類似工具，並成功分析出憑證，減小暴露風險。
4. 讀取分析sysinfo時的思路被驗證可行(strings→ltrace→2>&1→grep)。
5. 路徑劫持已經熟練，沒什麼難度。

### 浪費時間的部分：

1. 無特別難懂的地方，最花時間的是手動嘗試上傳繞過的時候。

### 新知識點：

1. 用簡單的cat即可把PHP腳本塞入圖檔，以前都是用特別工具處理。

### 與實戰對應：

1. 即使是靶機環境，也要盡量使用Live off the land原則，不要輕易做一些會增加暴露風險的事。
