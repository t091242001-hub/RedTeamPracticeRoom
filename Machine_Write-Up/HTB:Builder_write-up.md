...
### 主要發現：
1. 漏洞 ：透過Jenkins控制器解析命令時的缺陷讀取伺服器上的任意檔案（風險等級：Critical）
2. 漏洞 ：透過Jenkins內部的SSH金鑰與管理功能執行遠端命令（風險等級：Critical）

### 攻擊鏈摘要：

經過資訊蒐集，目標IP有Jenkins服務並被發現運行版本存在公開漏洞(CVE-2024-23897)，該漏洞允許未經身份驗證的使用者讀取控制器檔案系統上的任意檔案。攻擊者能夠提取Jenkins用戶的用戶名和密碼雜湊。利用這些憑證登入Jenkins服務，攻擊者可以利用加密的SSH金鑰來執行遠端程式碼，讀取伺服器的SSH公鑰，並取得伺服器的root權限。


### 潛在影響：

1. 舊版本公開漏洞將導致伺服器內機密資料，包含原始碼、部署金鑰、客戶資料及伺服器憑證外洩。
2. 獲得權限的攻擊者可能在建置流程中植入惡意程式碼，影響下游用戶，造成供應鏈投毒，影響企業信任度。
3. 控制面板託管root權限的SSH私鑰，可被攻擊者利用導致任意程式碼執行，造成系統全面癱瘓。


### 修復建議：

1. 立即更新Jenkins至官方修復版本。
2. 立即更換所有受影響的用戶密碼、SSH金鑰及Jenkins內的API Token。
3. 限制Jenkins服務執行帳號的系統權限，避免其具備直接取得root權限的路徑。
4. 除非必要，不應將Jenkins管理介面直接暴露於公網，建議透過VPN或存取控制清單保護。
