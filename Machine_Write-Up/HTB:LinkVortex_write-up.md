...
### 主要發現：
1. 漏洞 ：透過暴露的敏感目錄.git中未被妥善處理的憑證訊息獲得管理頁面的存儲權限（風險等級：Critical）
2. 漏洞 ：利用舊版本公開漏洞達成任意檔案讀取獲得系統使用者SSH憑證（風險等級：High）
3. 漏洞 ：透過未設密碼的sudo權限利用缺陷腳本達成權限提升（風險等級：Critical）

### 攻擊鏈摘要：

經過資訊蒐集後，發現存在Ghost內容管理系統以及dev子網域，在dev子網域中有暴露的.git目錄，其中藏有Ghost內容管理系統的明文憑證。利用憑證登入Ghost內容管理系統後，可了解該系統版本過舊，存在CVE-202340028公開漏洞，該漏洞允許已認證用戶上傳符號連結，從而在Ghost容器內讀取任意檔案。攻擊者可以利用Ghost設定檔中暴露的憑證，取得系統存儲權限。使用者有無須密碼的sudo權限，可執行多重漏洞的腳本，透過該腳本權限提升至root權限。


### 潛在影響：

1. 暴露的.git目錄會洩漏重要資訊，包含內部原始碼、明文憑證，造成管理員權限洩漏。
2. 內容管理系統版本過舊，存在公開漏洞，將造成內部文件洩漏、權限外洩。
3. 有多重漏洞的腳本擁有無須密碼的sudo權限，使攻擊者可輕易權限提升至root，造成伺服器全面淪陷。
4. 攻擊鏈完整，攻擊者最終可取得root權限，可完全控制伺服器，包括讀取、修改或刪除所有用戶資料。

### 修復建議：

1. 立即禁止公網存取所有環境的.git目錄，建議在Web伺服器設定攔截規則。
2. 徹底刪除Git歷史中的敏感憑證，並立即更換所有受影響的密碼。
3. 立即升級Ghost CMS更新至最新穩定版本。
4. 檢視Ghost容器的權限，確保其無法讀取非必要的系統設定檔。
5. 立刻修正sudo腳本或取消其sudo權限，若非必要，否則應移除普通使用者執行高風險系統腳本的無密碼sudo權限。
...
1. 攻擊鏈清晰，漏洞複現容易。
2. 以全程手動方式實現漏洞利用。
3. sudo腳本存在三種漏洞，讀懂後其實不難，收穫很多。

### 浪費時間的部分：

1. 新版kali安裝git-dumper有點花時間。


### 新知識點：

1. 符號連結檔案的相關漏洞是第一次接觸。
2. 從腳本挖掘出邏輯漏洞，若非我仔細研讀，否則我就用命令注入來提權了。
