...
### 主要發現：
1. 漏洞 ：透過SNMP服務配置錯誤取得敏感的使用者明文憑證（風險等級：High）
2. 漏洞 ：透過內部運行的PandoraFMS利用已公開的SQL注入漏洞繞過驗證並利用遠端程式碼執行漏洞取得主機存取權限（風險等級：Critical）
3. 漏洞 ：劫持SUID程式中使用相對路徑的外部指令達成權限提升。（風險等級：High）

### 攻擊鏈摘要：

連接埠掃描顯示該主機上執行SSH、Web伺服器和SNMP服務。攻擊者首先透過列舉SNMP服務取得初始立足點，枚舉結果顯示了使用者的明文憑。主機枚舉顯示PandoraFMS運行在內部連接埠上，可透過連接埠轉送存取。攻擊者利用PandoraFMS服務中的SQL注入和遠端程式碼執行漏洞，橫向移動到另一個使用者。最後，攻擊者利用SUID二進位檔案注入PATH變量，將權限提升到root。

### 潛在影響：

1. 未加密的SNMP服務暴露系統架構資訊與使用者身分，導致帳號密碼外洩。
2. 繞過邊界防火牆限制，使原本僅限於內部存取的敏感服務暴露於外部攻擊風險。
3. PandoraFMS版本過舊，公開漏洞太多，攻擊者可輕易取得該應用程式使用者的系統權限與系統權限，實現橫向移動。
4. 攻擊者從受限的使用者權限跨越至root最高權限，取得伺服器的絕對控制權，可植入後門、清除日誌或竊取全伺服器資料。


### 修復建議：

1. 升級至SNMPv3，啟用加密與使用者驗證，並限制存取的來源IP。
2. 更新PandoraFMS至官方安全修補版本，移除SQLi與RCE風險。
3. 重新檢查所有SUID/SGID檔案。開發具備高權限的程式時，必須在代碼中使用絕對路徑呼叫外部命令。
